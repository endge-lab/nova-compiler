...
export interface NovaStandaloneRuntimeModuleOptions {
  options?: Record<string, unknown> | (() => Record<string, unknown>)
  plugins?: Array<(registry: unknown) => void>
}

export interface NovaStandaloneAppOptions extends NovaVitePluginOptions {
  entry: string
...
const NOVA_STANDALONE_ENTRY_ID = 'virtual:nova-standalone-app-entry'
const NOVA_STANDALONE_PUBLIC_ENTRY_ID = '/__nova_standalone_app_entry.js'
const RESOLVED_NOVA_STANDALONE_ENTRY_ID = `\0${NOVA_STANDALONE_ENTRY_ID}`

/** Vite preset для fullscreen Nova-приложения без Vue host. */
export function novaStandaloneApp(options: NovaStandaloneAppOptions): Array<Plugin> {
  return [
    novaVitePlugin(options),
    createNovaStandaloneAppPlugin(options),
  ]
}

function createNovaStandaloneAppPlugin(options: NovaStandaloneAppOptions): Plugin {
  let root = process.cwd()

  return {
    name: 'endge-nova-standalone-app',
    enforce: 'post',

    /**
     * Запоминает итоговый root Vite-проекта для абсолютных import ids.
     */
    ...
    /**
     * Резолвит virtual entry standalone-приложения.
     */
    resolveId(id) {
      if (id === NOVA_STANDALONE_ENTRY_ID || id === NOVA_STANDALONE_PUBLIC_ENTRY_ID) {
        return RESOLVED_NOVA_STANDALONE_ENTRY_ID
      }
      return null
    },

    /**
     * Генерирует browser entry, который создает canvas и монтирует app.nova.
     */
    load(id) {
      if (id !== RESOLVED_NOVA_STANDALONE_ENTRY_ID) return null
      return generateNovaStandaloneEntry(options, root)
    },

    /**
     * Подключает standalone entry без пользовательского main.ts.
     */
    transformIndexHtml: {
      order: 'pre',
      handler() {
        return [{
          ...
          attrs: {
            type: 'module',
            src: NOVA_STANDALONE_PUBLIC_ENTRY_ID,
          },
          injectTo: 'body',
        }]
      },
...
function generateNovaStandaloneEntry(options: NovaStandaloneAppOptions, root: string): string {
  const entryPath = resolveStandaloneRequest(options.entry, root)
  const runtimeImport = options.runtime
    ? `import * as __novaStandaloneRuntime from ${JSON.stringify(resolveStandaloneRequest(options.runtime, root))};`
    : 'const __novaStandaloneRuntime = {};'
  const appOptions = serializeStandaloneOptions(options.options ?? {})
  const surfaceName = options.surfaceName ?? 'nova-standalone'
  const canvasLabel = options.canvasLabel ?? 'Nova standalone app'

  return `import { Nova } from '@endge/nova';
import __NovaStandaloneRoot from ${JSON.stringify(entryPath)};
${runtimeImport}

const __novaStandaloneBaseOptions = ${appOptions};
const __novaStandaloneCanvasLabel = ${JSON.stringify(canvasLabel)};
const __novaStandaloneSurfaceName = ${JSON.stringify(surfaceName)};
const __novaStandaloneStage = document.createElement('div');
const __novaStandaloneCanvas = document.createElement('canvas');
let __novaStandaloneApp = null;
let __novaStandaloneSurface = null;
let __novaStandaloneMountHandle = null;
let __novaStandaloneScope = Nova.createScope();
...
__novaStandaloneStage.setAttribute('data-nova-standalone-app', __novaStandaloneCanvasLabel);
__novaStandaloneStage.style.position = 'fixed';
__novaStandaloneStage.style.inset = '0';
__novaStandaloneStage.style.overflow = 'hidden';
__novaStandaloneCanvas.setAttribute('aria-label', __novaStandaloneCanvasLabel);
__novaStandaloneCanvas.style.display = 'block';
__novaStandaloneCanvas.style.width = '100%';
__novaStandaloneCanvas.style.height = '100%';
__novaStandaloneStage.appendChild(__novaStandaloneCanvas);
document.body.appendChild(__novaStandaloneStage);

function __novaStandaloneRuntimeOptions() {
  const value = __novaStandaloneRuntime.options;
  return typeof value === 'function' ? value() : value ?? {};
}

function __novaStandaloneRuntimePlugins() {
  return Array.isArray(__novaStandaloneRuntime.plugins) ? __novaStandaloneRuntime.plugins : [];
}

function __novaStandaloneReadSize() {
  return {
    width: Math.max(1, Math.floor(__novaStandaloneStage.clientWidth || window.innerWidth || 1)),
    height: Math.max(1, Math.floor(__novaStandaloneStage.clientHeight || window.innerHeight || 1)),
  };
}

function __novaStandaloneCreateOptions(size) {
  const runtimeOptions = __novaStandaloneRuntimeOptions();
  return {
    ...__novaStandaloneBaseOptions,
    ...runtimeOptions,
    target: __novaStandaloneCanvas,
    size: {
      ...(__novaStandaloneBaseOptions.size ?? {}),
      ...(runtimeOptions.size ?? {}),
      width: size.width,
      height: size.height,
    },
...
function __novaStandaloneTemplateProps(size) {
  return {
    width: size.width,
    height: size.height,
  };
...
function __novaStandaloneMount() {
  const size = __novaStandaloneReadSize();
  __novaStandaloneApp = Nova.createApp(__novaStandaloneCreateOptions(size));
  for (const plugin of __novaStandaloneRuntimePlugins()) {
    plugin(__novaStandaloneApp.schema);
  }
  __novaStandaloneSurface = __novaStandaloneApp.createSurface(__novaStandaloneSurfaceName);
  __novaStandaloneScope = Nova.createScope();
  __novaStandaloneMountHandle = Nova.mount(__NovaStandaloneRoot, {
    app: __novaStandaloneApp,
    surface: __novaStandaloneSurface,
    scope: __novaStandaloneScope,
    props: __novaStandaloneTemplateProps(size),
  });
}

function __novaStandaloneResize() {
  if (!__novaStandaloneApp || !__novaStandaloneMountHandle) return;
  const size = __novaStandaloneReadSize();
  __novaStandaloneApp.options(size);
  __novaStandaloneMountHandle.updateProps(__novaStandaloneTemplateProps(size));
}

function __novaStandaloneDestroy() {
  window.removeEventListener('resize', __novaStandaloneResize);
  try {
    __novaStandaloneMountHandle?.destroy();
    __novaStandaloneApp?.destroy();
  } finally {
    __novaStandaloneMountHandle = null;
    __novaStandaloneSurface = null;
    __novaStandaloneApp = null;
    __novaStandaloneScope = Nova.createScope();
  }
}

__novaStandaloneMount();
window.addEventListener('resize', __novaStandaloneResize);
window.addEventListener('beforeunload', __novaStandaloneDestroy, { once: true });
`;
}

function resolveStandaloneRequest(request: string, root: string): string {
  return normalizePath(path.isAbsolute(request) ? request : path.resolve(root, request))
}

function serializeStandaloneOptions(options: Record<string, unknown>): string {
  return JSON.stringify(options, (_key, value) => {
    if (typeof value === 'function') {
      throw new Error('novaStandaloneApp options must be serializable. Move runtime functions to runtime module.')
    }
    return value
  }, 2)
}
