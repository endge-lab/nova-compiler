...
function transformVueNovaCanvasTemplates(
  ...
  walkVueTemplateElements(ast, node => {
    ...
    const defaultChildren = node.children.filter((child: any) => !isNamedVueTemplate(child) && !isEmptyTextNode(child))
    ...
    const bindingAttrs = normalizedTemplate.bindings
      .map(name => ` :${toKebabCase(name)}="${name}"`)
      .join('')
...
function isSingleTopLevelNovaRoot(templateSource: string): boolean {
  try {
    ...
    const root = ast.children.find((child: any) => child.type === NodeTypes.ELEMENT && child.tag === 'template')
...
function normalizeInlineVueTemplateBindings(templateSource: string): { source: string; bindings: Array<string> } {
